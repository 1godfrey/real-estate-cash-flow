...
def get_sample_properties(zip_code: str, min_coc_return: float, down_payment: float, interest_rate: float, loan_term: int, monthly_expenses: float):
    ...
    for i in range(num_properties):
        ...
        annual_cashflow_for_coc = (price * (down_payment / 100) * (min_coc_return / 100))
        ...
        if metrics['cash_on_cash_return'] >= min_coc_return * 0.5 and metrics['cash_flow'] >= 100:
    ...
    try:
        ...
        for zip_code in unique_zip_list:
            ...
            try:
                ...
                logging.warning(f"API failed for {zip_code}, using sample properties: {str(api_error)}")
                api_failures += 1
        ...
        if api_failures > 0:
            flash(f"Used sample data for {api_failures} ZIP codes where API failed", 'warning')
